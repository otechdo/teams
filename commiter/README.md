...
- Install using Cargo: `cargo install zuu commiter cargo-watch`
- It's optimised for Rust projects, so run `cargo login` first if you're working with a Cargo project.
- Watches your project and helps you create well-formatted commit messages using cosmic commit types.

**Git Fame:**

- Install using pip: `pip install git-fame`
- Allows you to analyse contributions and rank authors based on various metrics.
- Define an alias for easier access: `git config --global alias.rank 'fame -s'`
...
* Big Bang: Initial project creation
    ...
    |\
    | *--* Black Hole(user): Remove deprecated user management module
    | *--* Wormhole(payment): Merge feature/payment-integration branch
    |    |\
